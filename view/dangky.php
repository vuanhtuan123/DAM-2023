<?php
$erro =[];
//Kiểm tra xem biến phiên $_SESSION có tên là 'error_password' có tồn tại và không rỗng hay không.
//Nếu có, gán giá trị của nó vào mảng $erro với khóa là 'password', sau đó xóa giá trị của biến để tránh hiển thị lỗi này lần nữa.
if(isset( $_SESSION['error_password'])&&( $_SESSION['error_password']!="")){ 
    //Gán giá trị của biến phiên $_SESSION['error_password'] vào mảng $erro với khóa là 'password'.
$erro['password'] =  $_SESSION['error_password']; 
unset($_SESSION['error_password']);//Xóa giá trị của biến phiên $_SESSION['error_password'] để tránh hiển thị lỗi này lần nữa.
  }
  if(isset($_SESSION['error_usename'])&&($_SESSION['error_usename']!="")){ 
    $erro['username'] = $_SESSION['error_usename'];
    unset($_SESSION['error_usename']); //Xóa giá trị của biến phiên $_SESSION['error_usename'] để tránh hiển thị lỗi này lần nữa.
      }
      if(isset( $_SESSION['error_email'])&&( $_SESSION['error_email']!="")){ 
        $erro['email'] =  $_SESSION['error_email']; //Gán giá trị của biến phiên $_SESSION['error_email'] vào mảng $erro với khóa là 'email'.
